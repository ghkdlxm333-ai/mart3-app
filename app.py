...
def load_master_data(file_path):
    try:
        ...
        # 2. 제품명 시트 처리 (C:D 범위 및 상품명 매핑 보강)
        prod_sheet = sheet_map.get('제품명')
        p_map, n_map = {}, {}
        if prod_sheet:
            # 전체 범위를 읽어오기 위해 header=None으로 읽은 후 컬럼 탐색
...
# --- 메인 실행부 ---
st.title("🛒🟢 이마트 계열 수주 자동화 (상품명/순서 최종 수정)")
...
if status_ok:
    ...
    if uploaded_file:
        try:
            ...
            st.success("✅ 상품명 매칭 및 컬럼 순서 정렬 완료")
            ...
            st.download_button(
                ...
                file_name=f"Order_Final_Fixed_{datetime.now().strftime('%m%d')}.xlsx"
            )
